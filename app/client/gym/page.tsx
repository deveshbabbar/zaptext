...
export default async function GymOverviewPage() {
  ...
  return (
    <>
      <PageTopbar
        crumbs={<>Gym / <b className="text-foreground">Overview</b></>}
        actions={<Pill variant="ink" href="/client/gym/plans">Manage plans</Pill>}
      />
      <div style={{ padding: '28px 32px 60px' }}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          ...
          <Kpi label="Confirmed today" value={todayClasses.filter((b) => b.status === 'confirmed').length} />
          <Kpi label="Pending approvals" value={pending.length} />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          <Panel title="Quick actions">
            <div className="grid grid-cols-2 gap-2.5">
              <Pill variant="ghost" href="/client/gym/plans">💪 Plans ({plans.length})</Pill>
              <Pill variant="ghost" href="/client/gym/schedule">📅 Schedule ({todayClasses.length})</Pill>
              <Pill variant="ghost" href="/client/staff">👥 Trainers</Pill>
              <Pill variant="ghost" href="/client/settings">⚙️ Bot settings</Pill>
            </div>
          </Panel>
          <Panel title="Today's classes" sub={`${todayClasses.length} for ${today}`}>
            {todayClasses.length === 0 ? (
              <div className="text-[13px] text-[var(--mute)] py-2">No classes booked for today.</div>
            ) : (
              <ul className="flex flex-col">
                {todayClasses.slice(0, 6).map((b) => (
